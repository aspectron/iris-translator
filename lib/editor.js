...
function Editor(translator, options, callback) {
    ...
    function initConfig(options, callback) {
        ...
        if (options.transifex) {
            ...
            if (!options.transifex.projectSlug) {
                throw new Error('Project slug is not being passed')
            }
    ...
    function initExpress(callback) {
        ...
        self.app.use(function (req, res, next) {
            ...
            if (user) {
                if (self.users[user.name] && user.pass == self.users[user.name]) {
                    return next();
                }
            }
    ...
    function initHttpServer(callback) {
        ...
        self.app.delete(self.baseUrl + '/entry/:hash', function (req, res) {
        ...
        self.app.get(self.baseUrl + '/export', function (req, res) {
            if (!self.transifex.available) {
                console.error('Translator: sync with Transifex is not configured');
                return res.send(405);
            }
        ...
        self.app.get(self.baseUrl + '/import', function (req, res) {
            if (!self.transifex.available) {
                console.log('Translator: sync with Transifex is not configured');
                return res.send(405);
            }
            ...
            transifex.getProject(projectSlug, function (err, project) {
                ...
                if (!project) {
                    console.error('Transifex Import: Project', projectSlug, 'not found');
                    return res.send(500);
                }
                            ...
                            if (entries[key]) {
                                ...
                                if (!existInStore) {
                                    try {
                                        ...
                                        console.log('Transifex Import: entry with', key, ' is not added', translationsForImport[key]);
                                    }
                                }
                            }
                        }
