...
function Editor (translator, options, callback) {
    ...
    function initHttpServer (callback) {
        ...
        self.app.get(self.baseUrl + '/enable/:locale', function (req, res) {
            ...
            var storage = new Storage({filename: translator.languagesFileName, directory: translator._storagePath});

            storage.setTranslations(translator.languages, function (err) {
                if (err) {
                    // rollback
                    self.switchLanguage(locale);
                    return res.send(500);
