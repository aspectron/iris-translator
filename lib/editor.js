...
function Editor (translator, options, callback) {
    ...
    function initHttpServer (callback) {
        ...
        self.app.get(self.baseUrl + '/enable/:locale', function (req, res) {
            ...
            var storage = new Storage({filename: translator.languagesFileName, storagePath: translator._storagePath});

            storage.setData(translator.languages, function (err) {
                if (err) {
                    // rollback
                    self.switchLanguage(locale);
                    return res.send(500);
        ...
        self.app.put(self.baseUrl + '/entry', function (req, res) {
            ...
            if (!entry.hash || !translator.i18n.entries[entry.hash]) return res.send(404);

            var o = translator.i18n.entries[entry.hash];

            _.each(entry.locale, function (text, code) {
                if (!translator.languages[code]) {
            ...
            o.multiline = entry.multiline == 'true' ? true : false;

            console.log("Updating entry:", o);

            if (self.ACTIVE_FLUSH) {
                translator.i18n.storeEntries(finish);
            } else {
                finish();
            }
        ...
        self.app.del(self.baseUrl + '/entry/:hash', function (req, res) {
            ...
            if (!translator.i18n.entries[hash]) return res.send(404);

            console.log("Deleting entry:", hash);

            delete translator.i18n.entries[hash];
        ...
        self.app.del(self.baseUrl + '/entry/:hash/:locale', function (req, res) {
            var hash = req.params.hash;
            var locale = req.params.locale;

            if (!translator.i18n.entries[hash]) return res.send(404);

            if (translator.i18n.entries[hash].locale[locale]) {
                console.log("Deleting locale", locale, "in entry:", hash);

                delete translator.i18n.entries[hash].locale[locale];
