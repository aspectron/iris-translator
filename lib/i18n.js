...
function i18n(options) {
    ...
    self.init = function (opt, callback) {
        if (opt.sourceLanguage) {
            self.sourceLanguage = opt.sourceLanguage;
        }
        self.fileStorage.setFilename(self.filename);

        if (opt.storagePath) {
            self.fileStorage.setDirectory(opt.storagePath);
        }

        self.providerRootFolderPath = opt.rootFolderPath ? opt.rootFolderPath + '/' : __dirname + '/../';
        self.providerFolders = opt.folders;

        if (_.isArray(opt.fileExtensions)) {
            self.fileExtensions = opt.fileExtensions;
        }
        ...
        if (opt.mongoStorage) {
            ...
            self.mongoStorage.init({
                url: opt.mongoStorage.url,
                translationCollectionName: opt.mongoStorage.translationCollectionName,
                languageCollectionName: opt.mongoStorage.languageCollectionName,
