...
var TranslationMongoStorage = require('./translationMongoStorage');
...
function i18n(options, callback) {
     ...
     */
    self.sourceLanguage = options.sourceLanguage || 'en';
    ...
    self.providerRootFolderPath = options.rootFolderPath ? path.normalize(options.rootFolderPath + '/') : __dirname + '/../';
    self.providerFolders = options.folders;

    if (_.isArray(options.fileExtensions)) {
        self.fileExtensions = options.fileExtensions;
    }

    function initFileStorage(callback) {
        self.fileStorage = new Storage(options);

        if (!options.filename) {
            self.fileStorage.setFilename(self.filename);
        }

        self.fileStorage.fileExists(function (exists) {
            if (exists) {
                callback();
            } else {
                self.fileStorage.createFile(function (err) {
                    callback();
                });
            }
        });
    ...
    function initMongoStorage(callback) {
        self.mongoStorage = new TranslationMongoStorage({
            url: options.mongoStorage.url,
            translationCollectionName: options.mongoStorage.translationCollectionName,
    ...
    /**
     * TODO VRuden must be merge or complete replacement?
     * Merge i18e.entries from file and mongodb
     *
     * @param callback
     */
    self.refreshEntries = function (callback) {
        self.fileStorage.getData(function (fileEntries) {
            if (self.mongoStorage) {
                self.mongoStorage.getTranslations(function (err, mongoEntries) {
                    if (err) {
                        return returnData(fileEntries);
                    }

                    return returnData(self.mergeEntriesFromDifferentStores(fileEntries, mongoEntries));
                });
            } else {
                returnData(fileEntries);
            }

            function returnData(entries) {
    ...
    self.mergeEntriesFromDifferentStores = function (fileEntries, mongoEntries) {
        if (!_.size(mongoEntries)) return fileEntries;
        if (!_.size(fileEntries)) return mongoEntries;

        _.each(mongoEntries, function (entry) {
            if (fileEntries[entry.hash] && fileEntries[entry.hash].updated > entry.updated) {
                entry = fileEntries[entry.hash];
            }
        });
    ...
    self.storeEntries = function (onlyFileStorage, callback) {
        if (typeof onlyFileStorage === 'function') {
            callback = onlyFileStorage;
            onlyFileStorage = false;
        }

        self.fileStorage.setData(self.entries, function (err) {
            // don't wait a saving to mongodb
            callback(err);

            if (!onlyFileStorage && self.mongoStorage) {
                self.mongoStorage.setTranslations(self.entries, function (error, result) {
    ...
    self.translate = function (message, params, category, locale) {
        locale = locale || self.sourceLanguage;
    ...
    self.createEntry = function (message, category, file) {
        ...
        if (!self.entries[hash]) {
            ...
            category = category ? category : self.basicCategory;
     ...
     *
     * @param locale
     * @param transifexSourceKey
     * @returns {*}
     */
    self.createTransifexEntry = function (locale, transifexSourceKey) {
        if (!locale[self.sourceLanguage]) throw new Error('Source message not found');

        var hash = crypto.createHash('sha1').update(locale[self.sourceLanguage]).digest('hex');

        self.entries[hash] = {
            hash: hash,
            transifexSourceKey: transifexSourceKey,
        ...
        console.log('Creating new entry', self.entries[hash].locale[self.sourceLanguage]);
    };

    self.updateEntry = function (entry, callback) {
        if (!entry.hash || !self.entries[entry.hash]) return callback('Entry not found');
    ...
    self.removeEntry = function (hash, callback) {
        if (!self.entries[hash]) return callback('Entry not found');

        delete self.entries[hash];
        console.log("Deleting entry:", hash);

        self.storeEntries(true, function (err) {
            callback(err);
        });

        if (self.mongoStorage) {
            self.mongoStorage.removeTranslation(hash, function (err) {
            ...
            digestFiles(files, function (err) {
                self.storeEntries(function () {
                    ...
                    /*var orphanEntries = [];
                     ...
                     if (!entry) {
                     _.each(self.entries, function (entry) {
                     var files = entry.files.map(function (i) {
                     return self.getRelativePath(i)
                     });
    ...
    self.getFilePath = function () {
        return self.fileStorage.getFilePath();
    };
...
//                console.log('Scanning:: Finished with result', result);
    ...
    function acceptFile(path, relative, fileExtensions) {
        ...
        return _.contains(fileExtensions, ext)
    }
...
//        console.log('Digesting file', file);
    ...
    function run() {
        ...
        steps.push(initFileStorage);

        if (options.mongoStorage) {
            steps.push(initMongoStorage);
        }

        steps.push(self.refreshEntries);
        steps.run(function (err) {
            if (err) throw err;

            self.importFromFolder(callback);
        });
    }
