...
var i18n = module.exports = {
    ...
    secondaryLanguageTpl:'--{text}--',
    basicCategory: 'basic',

    fileExtensions: ['js', 'ejs'],
    ...
    init: function (opt, callback) {
        ...
        if (opt.storagePath) {
            Storage.setDirectory(opt.storagePath);
        }
    ...
    refreshEntries: function (callback) {
        var self = this;

        Storage.getData(function (entries) {
    ...
    storeEntries: function (callback) {
        Storage.setData(this.entries, callback);
    },
     ...
     */
    translate: function (message, params, category, locale) {
        var self = this;
        ...
        if (entry) {
            ...
            self.createEntry(message, category);
        }
    ...
    createEntry: function (message, category, file) {
        var self = this;
    ...
    importFromFolder: function (callback) {
        var self = this;

        console.log("Doing import...");

        if(_.isObject(self.providerFolders)){
            var tmpFolder = [];
            var tmpExtensions = [];
        ...
        console.log("Translator digesting folders:", folders);
     ...
     */
    onMissingTranslation: function (callback) {
...
function scanFolders(rootFolderPath, folders, folderFileExtensions, callback) {
    ...
    var fileExtensions = i18n.fileExtensions.slice();
    scanFolder();
...
function digestFile(file, callback) {
    ...
    fs.readFile(file, {encoding: 'utf8'}, function (err, data) {
        ...
        _.each(matches, function (match) {
            if (match) {
                i18n.createEntry(match.substring(4, match.length - 2), '', file);
            }
        });
        ...
        _.each(matches, function (match) {
            if (match) {
                i18n.createEntry(match.substring(1, match.length - 4), '', file);
            }
        });
