...
function i18n(options, callback) {
    ...
    self.storage = new Storage(options);
    ...
    function init () {
        ...
        self.storage.setFilename(self.filename);

        if (options.storagePath) {
            self.storage.setDirectory(options.storagePath);
        }
        ...
        self.storage.fileExists(function (exists) {
            if (exists) {
                self.importFromFolder(callback);
            } else {
                self.storage.createFile(function (err) {
                    self.importFromFolder(callback);
                });
            }
        });
    ...
    self.storeEntries = function (callback) {
        self.storage.setData(self.entries, callback);
    }
