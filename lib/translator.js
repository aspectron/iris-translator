...
/*
 var config = utils.get_config('translator');
 _.each(config.languages, function (o, code) {
 o.code = code;
 })
...
var Translator = module.exports = {
    languages: {"en": {name: "English", enabled: true}},
    defaultBaseUrl: 'tr',
    port: 32032,
    users: {
    ...
    // middleware
    useSession: function (req, res, next) {
        var locale;
        if (req.session) {
        ...
        req._T = function (message, loc) {
            loc = loc ? loc : locale;
            return i18n.translate(message, '', '', loc);
        }
    ...
    // middleware
    useUrl: function (req, res, next) {
        var locale = req.param('locale'); //req.params.locale;

        if (!_.has(Translator.languages, locale)) {
            locale = defaultLanguage;
        }
    ...
    // middleware
    toggleLanguage: function (req, res, next) {
        if (_.has(Translator.languages, req.params.locale)) {
    ...
    activateLanguage: function (locale, req) {
        if (_.has(Translator.languages, locale)) {
    ...
    // middleware
    actionGetLanguages: function (req, res) {
        res.send(200, Translator.languages);
    },

    // middleware
    actionGetEnabledLanguages: function (req, res) {
    ...
    separateEditor: function () {
        if (buildInEditorIsRunning) return false;

        var self = this;

        var translationFilePath = path.normalize(i18n.getFilePath());
        ...
        watcher1.on('change', function (path) {
            ...
            i18n.refreshEntries(function () {
                //console.log(i18n.entries);
            });
        });
    ...
    runEditor: function (options, callback) {
...
//        });
        ...
        self.app.get(self.baseUrl + '/rescan', function (req, res, next) {
            i18n.importFromFolder(function () {
                res.redirect(self.baseUrl + '/');
            });
        });
        ...
        function getEntries(query, skip, limit) {
            ...
            if (!query) {
                ...
                _.each(i18n.entries, function (entry, index) {
        ...
        self.app.put(self.baseUrl + '/entry', function (req, res, next) {
            ...
            if (!entry.hash || !i18n.entries[entry.hash]) return res.send(404);

            var o = i18n.entries[entry.hash];
        ...
        self.app.del(self.baseUrl + '/entry/:hash', function (req, res, next) {
            ...
            if (!i18n.entries[hash]) return res.send(404);

            console.log("Deleting entry:", hash);

            delete i18n.entries[hash];
        ...
        self.app.del(self.baseUrl + '/entry/:hash/:locale', function (req, res, next) {
            ...
            if (i18n.entries[hash].locale[locale]) {
                console.log("Deleting locale", locale, "in entry:", hash);

                delete i18n.entries[hash].locale[locale];
        ...
        self.app.get(self.baseUrl + '/export', function (req, res, next) {
            ...
            transifex.getProject(projectSlug, function (err, project) {
                ...
                transifex.getResource(projectSlug, resourceSlug, function (err, resource) {
                    ...
                    _.each(i18n.entries, function (entry) {
                        //if (!entry.orphan) {
                            for (var locale in entry.locale) {
